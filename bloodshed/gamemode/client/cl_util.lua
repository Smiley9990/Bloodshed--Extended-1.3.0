...
local function CreateFinishPanel(type, isvote, classMap)
	...
	for _, ply in player.Iterator() do
		local nwc = (IsValid(ply) and classMap[ply]) or ply:GetNW2String("Class")
		...
		if MuR.GamemodeCount == 54 and nwc == "Combine" then
			...
			classname = MuR.Language[otherKeys[combineType]] or MuR.Language["combine"]
		elseif MuR.GamemodeCount == 54 and nwc == "Rebel" then
			...
			classname = MuR.Language[otherKeys[rebelType]] or MuR.Language["rebel"]
		else
...
net.Receive("MuR.FinalScreen", function()
	...
	for i = 1, count do
		...
		if IsValid(ent) and ent:IsPlayer() and cls ~= "" then
			classMap[ent] = cls
		end
	end
	hook.Call("MuR.OnFinalScreen", nil, str, vote)
	CreateFinishPanel(str, vote, classMap)
end)
