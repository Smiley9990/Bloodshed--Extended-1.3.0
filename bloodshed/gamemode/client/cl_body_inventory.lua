...
local function CreateDisarmMinigame(tripwire)
    if not IsValid(tripwire) then return end
    ...
    local title = vgui.Create("DLabel", minigameFrame)
    local titleText = MuR.Language["tripwire_disarm_minigame_title"] or "Disarming tripwire..."
    ...
    local instruction = vgui.Create("DLabel", minigameFrame)
    instruction:SetText(MuR.Language["tripwire_disarm_minigame_hint"] or "Press SPACE when slider is in green zone")
    instruction:SetFont("MuR_FontDef")
    instruction:SetTextColor(SEARCH_THEME.textDark)
    instruction:SetPos(We(20), He(52))
    instruction:SizeToContents()
    ...
    local function CheckHit()
        ...
        if sliderCenter >= targetStart and sliderCenter <= targetEnd then
            gameActive = false
            local successText = MuR.Language["tripwire_disarm_success"] or "Success! Disarming..."
            instruction:SetText(successText)
            instruction:SetTextColor(SEARCH_THEME.success)
            surface.PlaySound("buttons/button9.wav")
            ...
            timer.Simple(0.5, function()
                ...
                if IsValid(tripwire) then
                    net.Start("MuR.TripwireDisarmResult")
                    net.WriteEntity(tripwire)
                    net.WriteBool(true)
                    net.SendToServer()
                end
            end)
            ...
            gameActive = false
            local failText = MuR.Language["tripwire_disarm_fail"] or "Failed! Tripwire activated!"
            instruction:SetText(failText)
            instruction:SetTextColor(SEARCH_THEME.danger)
            surface.PlaySound("buttons/button10.wav")
    ...
    minigameFrame.OnKeyCodePressed = function(self, key)
        if key == KEY_SPACE then
            CheckHit()
        elseif key == KEY_ESCAPE or key == KEY_E then
            gameActive = false
            minigameFrame:AlphaTo(0, 0.2, 0, function()
                if IsValid(minigameFrame) then
                    minigameFrame:Remove()
                end
            end)
...
net.Receive("MuR.TripwireDisarm", function()
    local ent = net.ReadEntity()
    if IsValid(ent) and ent:GetClass() == "murwep_grenade" then
        CreateDisarmMinigame(ent)
    end
end)
