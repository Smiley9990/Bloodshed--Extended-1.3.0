...
hook.Add("Think", "CircleSelectorThink", function()
	local armorPanelOpen = MuR.ArmorPanelFrame and IsValid(MuR.ArmorPanelFrame)
	if input.IsKeyDown(KEY_Q) and fadeState < 1 and not clickProcessed and LocalPlayer():Alive() and not armorPanelOpen then
		fadeState = math.Clamp(fadeState + FrameTime() / fadeDuration, 0, 1)
		MuR.DrawHUD = false
		gui.EnableScreenClicker(true)
	elseif (not input.IsKeyDown(KEY_Q) or clickProcessed or !LocalPlayer():Alive() or armorPanelOpen) and fadeState > 0 then
		fadeState = math.Clamp(fadeState - FrameTime() / fadeDuration, 0, 1)

		if fadeState == 0 then
			if not armorPanelOpen then
				MuR.DrawHUD = true
...
hook.Add("HUDPaint", "CircleSelectorPaint", function()
	...
	if MuR.ArmorPanelFrame and IsValid(MuR.ArmorPanelFrame) then return end
	drawBackground()
	drawSelector()
end)
