...
hook.Add("HUDPaint", "MurderedHUD", function()
	...
	if MuR.DrawHUD and ply:Alive() and !MuR:GetClient("blsd_nohud") then
		...
		if IsValid(tr) and tr:GetPos():DistToSqr(ply:GetPos()) < 10000 and !ply:GetNW2Bool("IsUnconscious", false) then
			...
			if tr:GetClass() == "murwep_grenade" and IsValid(tr.StakeEnt) and not tr.Activated then
				local parsed = GetMarkup("<font=MuR_Font0><colour=255,255,255>E<colour=200,200,0>" .. (MuR.Language["tripwire_disarm_hint"] or " - Disarm tripwire"))
	...
	if ply:IsSuperAdmin() and ply:Alive() then
		local cvDebugHP = GetConVar("mur_debug_hp")
		if cvDebugHP and cvDebugHP:GetBool() then
			local hp = ply:Health()
			local maxHp = ply:GetMaxHealth()
			local frac = maxHp > 0 and (hp / maxHp) or 1
			local hpColor = Color(255 - frac * 255, frac * 255, 0)
			local hpText = "HP: " .. hp .. " / " .. maxHp
			draw.SimpleTextOutlined(hpText, "MuR_Font4", We(24), ScrH() - He(24), hpColor, TEXT_ALIGN_LEFT, TEXT_ALIGN_BOTTOM, 2, Color(0, 0, 0, 200))
		end
	end
end)
...
local function ShowAnnounce(type, ent)
	...
	elseif type == "money_cancel" then
		...
		font = "HomigradFontLarge"
	elseif type == "shop_disabled_mode50" then
		text = MuR.Language["shop_disabled_mode50"]
		font = "HomigradFontLarge"
	elseif type == "legion_victory" then
		text = MuR.Language["legion_victory"]
		font = "HomigradFontLarge"
...
local function Internal_ShowMessage(type, ent, callback)
	...
	elseif type == "tripwire_disarmed" then
		text = MuR.Language["tripwire_disarmed"] or ""
		font = "HomigradFont"
	elseif type == "tripwire_disarmed_success" then
		text = MuR.Language["tripwire_disarmed_success"] or ""
		font = "HomigradFont"
	elseif type == "tripwire_disarmed_fail" then
		text = MuR.Language["tripwire_disarmed_fail"] or ""
		font = "HomigradFont"
	else
...
hook.Add("HUDPaint", "ShowMessageMuR2_Layered", function()
    ...
    for i = #MuR.ActiveMessages2, 1, -1 do
        ...
        local clr = msg.color and Color(msg.color.r, msg.color.g, msg.color.b, 255 * scale) or Color(255, 255, 255, 255 * scale)
        drawTextRotated(partial, msg.font, bx, y, clr, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 0, scale, scale)
    end
end)

function MuR.ShowCenteredMessage(text, duration, color)
    ...
    timer.Simple(duration, function()
        if msg then msg.start = false msg.vanishStart = CurTime() end
    end)
end
...
net.Receive("MuR.Countdown", function()
	...
	if MuR.GamemodeCount == 54 and MuR.Data["Mode54Defender"] and MuR.Data["Mode54Defender"] > 0 then
		local defenderStartTime = CurTime()
		hook.Add("HUDPaint", "MuR_Mode54CountdownDefender", function()
			if MuR.GamemodeCount ~= 54 or not MuR.Data["Mode54Defender"] or MuR.Data["Mode54Defender"] <= 0 then
				hook.Remove("HUDPaint", "MuR_Mode54CountdownDefender")
				return
			end
			local defText = MuR.Data["Mode54Defender"] == 1 and (MuR.Language["mode54_combine_defend"] or "Combine держат оборону") or (MuR.Language["mode54_rebels_defend"] or "Повстанцы держат оборону")
			local pulse = 0.9 + math.sin(CurTime() * 1.2) * 0.1
			local defColor = MuR.Data["Mode54Defender"] == 1 and Color(80, 160, 255, 255 * pulse) or Color(255, 160, 60, 255 * pulse)
			local shadowColor = MuR.Data["Mode54Defender"] == 1 and Color(0, 80, 180, 120) or Color(180, 100, 20, 120)
			local elapsed = CurTime() - defenderStartTime
			local centerX, centerY = ScrW() / 2, He(320)
			local cornerX, cornerY = We(20), He(20)
			local moveDuration = 2
			local moveStart = 5
			local x, y
			if elapsed < moveStart then
				x, y = centerX, centerY
			elseif elapsed < moveStart + moveDuration then
				local t = (elapsed - moveStart) / moveDuration
				t = t * t * (3 - 2 * t)
				x = Lerp(t, centerX, cornerX)
				y = Lerp(t, centerY, cornerY)
			else
				x, y = cornerX, cornerY
			end
			local alignX = (elapsed < moveStart + moveDuration) and TEXT_ALIGN_CENTER or TEXT_ALIGN_LEFT
			local alignY = (elapsed < moveStart + moveDuration) and TEXT_ALIGN_CENTER or TEXT_ALIGN_TOP
			draw.SimpleText(defText, "MuR_Font2", x + 1, y + 1, shadowColor, alignX, alignY)
			draw.SimpleText(defText, "MuR_Font2", x, y, defColor, alignX, alignY)
		end)
	end
end)
...
hook.Add("CalcView", "ViewBob", function(ply, pos, angles, fov)
	...
	if IsValid(ply:GetNW2Entity("ControlDrone")) then return end
...
function MuR:ShowStartScreen(gamemode, class)
	...
	local gamemodeBG, gamemodeBG2
	if gamemode == 50 then
		gamemodeBG = Material("murdered/modes/gamemodess1.png", "")
		gamemodeBG2 = Material("murdered/modes/gamemodess1h.png", "")
	elseif gamemode == 51 then
		gamemodeBG = Material("murdered/modes/gamemodess2.png", "")
		gamemodeBG2 = Material("murdered/modes/gamemodess2h.png", "")
	elseif gamemode == 52 then
		gamemodeBG = Material("murdered/modes/gamemodess3.png", "")
		gamemodeBG2 = Material("murdered/modes/gamemodess3h.png", "")
	elseif gamemode == 53 then
		gamemodeBG = Material("murdered/modes/gamemodess4.png", "")
		gamemodeBG2 = Material("murdered/modes/gamemodess4h.png", "")
	elseif gamemode == 54 then
		gamemodeBG = Material("murdered/modes/gamemodess5.png", "")
		gamemodeBG2 = Material("murdered/modes/gamemodess5h.png", "")
	elseif gamemode == 56 then

		gamemodeBG = Material("murdered/modes/gamemode21.png", "")
		gamemodeBG2 = Material("murdered/modes/gamemode21h.png", "")
	else
		gamemodeBG = Material("murdered/modes/gamemode" .. gamemode .. ".png", "")
		gamemodeBG2 = Material("murdered/modes/gamemode" .. gamemode .. "h.png", "")
	end
    ...
    if gamemode == 54 and IsValid(ply) then
        if class == "Combine" then
            local combineType = ply:GetNW2Int("CombineType", 1)
            local otherKeys = {"combine_var_heavy", "combine_var_suppressor", "combine_var_captain", "combine_var_grunt"}
            classData.other = otherKeys[combineType] or "combine_var_heavy"
        elseif class == "Rebel" then
            local rebelType = ply:GetNW2Int("RebelType", 1)
            local otherKeys = {"rebel_var_hev", "rebel_var_crossbow", "rebel_var_demolitionist", "rebel_var_heavy", "rebel_var_normal"}
            classData.other = otherKeys[rebelType] or "rebel_var_normal"
        end
    end
    ...
    hook.Add("HUDPaint", "MurderedStartScreen", function()
        ...
        if MuR.CurrentStartScreenGamemode == 53 and not MuR.Mode53StartScreenWaiting then
            return
        end
    ...
    local function CloseStartScreen()
        if IsValid(ply) then
            ply:ScreenFade(SCREENFADE.IN, color_black, 1, 0)
        end
        if gamemode == 50 and MuR.Mode50Sound and IsValid(MuR.Mode50Sound) then
            MuR.Mode50Sound:Stop()
            MuR.Mode50Sound = nil
        end
        if gamemode == 51 and MuR.Mode51Sound and IsValid(MuR.Mode51Sound) then
            MuR.Mode51Sound:Stop()
            MuR.Mode51Sound = nil
        end
        if gamemode == 52 and MuR.Mode52Sound and IsValid(MuR.Mode52Sound) then
            MuR.Mode52Sound:Stop()
            MuR.Mode52Sound = nil
        end
        if gamemode == 53 and MuR.Mode53Sound and IsValid(MuR.Mode53Sound) then
            MuR.Mode53Sound:Stop()
            MuR.Mode53Sound = nil
        end
        if gamemode == 54 and MuR.Mode54Sound and IsValid(MuR.Mode54Sound) then
            MuR.Mode54Sound:Stop()
            MuR.Mode54Sound = nil
        end
        MuR.DrawHUD = true
	...
	if gamemode == 53 then
        MuR.CurrentStartScreenGamemode = 53
        MuR.Mode53StartScreenWaiting = true
        timer.Simple(30, function()
            if MuR.Mode53StartScreenWaiting and MuR.CurrentStartScreenGamemode == 53 then
                MuR.Mode53StartScreenWaiting = false
                MuR.CurrentStartScreenGamemode = nil
                CloseStartScreen()
            end
        end)
    else
        timer.Simple(12, CloseStartScreen)
    end
end
...
net.Receive("MuR.Mode53CloseStartScreen", function()
	local ply = LocalPlayer()
	if IsValid(ply) then
		ply:ScreenFade(SCREENFADE.IN, color_black, 1, 0)
	end
	MuR.DrawHUD = true
	hook.Remove("HUDPaint", "MurderedStartScreen")
	if MuR.Mode53Sound and IsValid(MuR.Mode53Sound) then
		MuR.Mode53Sound:Stop()
		MuR.Mode53Sound = nil
	end
	MuR.Mode53StartScreenWaiting = false
