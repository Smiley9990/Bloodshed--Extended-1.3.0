    ...
    OnModeStarted = function(mode)
        if SERVER then
            ...
            timer.Simple(0.5, function()
                ...
                local usedPositions = {}
                local minSpawnDist = 400
                local zoneCenterSum = Vector(0, 0, 0)
                local zoneCenterCount = 0

                for _, ply in ipairs(allPlayers) do
                    if ply:Alive() then
                        local pos = nil
                        for attempt = 1, 40 do
                            local candidate = MuR:GetRandomPos(nil, nil, nil, nil, true)
                            if isvector(candidate) then
                                local tooClose = false
                                for _, used in ipairs(usedPositions) do
                                    if used:DistToSqr(candidate) < minSpawnDist * minSpawnDist then
                                        tooClose = true
                                        break
                                    end
                                end
                        ...
                        if not pos then
                            pos = MuR:GetRandomPos(nil, nil, nil, nil, true) or ply:GetPos()
                        end
                        table.insert(usedPositions, pos)
                        MuR.Mode19.PlayerSpawnPositions[ply] = pos
                        ply:SetPos(pos + Vector(0, 0, 10))
                        zoneCenterSum = zoneCenterSum + pos
                        zoneCenterCount = zoneCenterCount + 1
                    end
                end

                local zoneCenter = (zoneCenterCount > 0) and (zoneCenterSum / zoneCenterCount) or (usedPositions[1] or Vector(0, 0, 0))
                MuR.Mode19.ZoneCenter = zoneCenter
                MuR.Mode19.SpawnPoint = zoneCenter

                local zone = ents.Create("bloodshed_zone")
                if IsValid(zone) then
                    zone:SetPos(zoneCenter)
    ...
    hook.Add("PlayerSpawn", "MuR_Mode19_Spawn", function(ply)
        if MuR.Gamemode == 19 and MuR.Mode19 then
            timer.Simple(0.1, function()
                ...
                local spawnPos = MuR.Mode19.PlayerSpawnPositions and MuR.Mode19.PlayerSpawnPositions[ply]
                if isvector(spawnPos) then
                    ply:SetPos(spawnPos + Vector(0, 0, 10))
                elseif MuR.Mode19.SpawnPoint then
                    local pos = MuR:GetRandomPos(nil, MuR.Mode19.SpawnPoint, 300, 3000, true)
