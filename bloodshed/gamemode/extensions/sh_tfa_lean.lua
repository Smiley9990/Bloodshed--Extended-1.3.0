...
local ron_lean_speed = 1
local general_lean_speed = 1
local unpredicted = false
local debugmode = false
local notify = false
local allow_crouch_leans = true
local always_allow_leaning = false
local auto_in_sights = true
local interp = 2
    ...
    for i, data in ipairs(binds) do
        ...
        if button == need_to_press then
            ...
            if typee == "hold" then
                ...
                if notify then ply:ChatPrint("[Leaning] Enabled "..pretty_name) end
            end
            ...
            if typee == "toggle" then
                ...
                if notify then ply:ChatPrint("[Leaning] Toggled "..pretty_name..": "..bool_to_str(state)) end
    ...
    for i, data in ipairs(binds) do
        ...
        if button == need_to_press then
            if typee == "hold" then
                ...
                if notify then ply:ChatPrint("[Leaning] Disabled "..pretty_name) end
            end
        end
    end
end)
...
hook.Add("SetupMove", "leaning_main", function(ply, mv, cmd)
    ...
    local canlean = always_allow_leaning or can_lean(ply)
    ...
    if leaning_left then
        fraction = Lerp(FrameTime() * 5 * general_lean_speed + FrameTime(), fraction, -1)
    end
    ...
    if leaning_ron then
        if cmd:KeyDown(IN_MOVELEFT) then
            fraction = Lerp(FrameTime() * 3 * ron_lean_speed * math.abs(fraction - 1) + FrameTime(), fraction, -1)
    ...
    if leaning_auto then
        ...
        local sanity = util.TraceHull({
            start = eyepos,
            endpos = eyepos + angles_forward * lean_amount,
            mask = MASK_BLOCKLOS,
            filter = ply,
        ...
        if sanity.Fraction < 0.99 then
            local left = util.TraceLine({
                start = eyepos,
                endpos = eyepos - angles_right * lean_amount,
                mask = MASK_BLOCKLOS,
                filter = ply
            })
            ...
            local left_forward = util.TraceLine({
                start = left.HitPos,
                endpos = left.HitPos + angles_forward * lean_amount * 2,
                mask = MASK_BLOCKLOS,
                filter = ply
            })
            ...
            local right_forward = util.TraceLine({
                start = right.HitPos,
                endpos = right.HitPos + angles_forward * lean_amount * 2,
                mask = MASK_BLOCKLOS,
                filter = ply
            })
        ...
        fraction = Lerp(FrameTime() * 5 * general_lean_speed + FrameTime(), fraction, wish_fraction)
    end

    if not leaning_left and not leaning_right and not leaning_ron and not leaning_auto then
        fraction = Lerp(FrameTime() * 5 * general_lean_speed + FrameTime(), fraction, 0)
    end
    ...
    local amount = fraction_smooth * lean_amount
...
if SERVER then
    hook.Add("Think", "leaning_bend", function()
        for k, ply in player.Iterator() do
            ...
            lean_bones(ply, ply:GetNW2Float("leaning_fraction_smooth") * lean_amount)
...
if CLIENT then
    ...
    concommand.Add("cl_lean_bind", function(ply, cmd, args, argstr)
        ...
        sights:SetValue(auto_in_sights)
