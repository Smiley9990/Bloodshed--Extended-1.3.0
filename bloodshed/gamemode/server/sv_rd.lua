...
function pl:CreateAdvancedRagdoll(withoutvel, impactVel)
	if self:GetNW2String("Class") == "Entity" then return end
	if MuR.Gamemode == 54 and self:GetNW2String("Class") == "Combine" and self:GetNW2Int("CombineType", 1) == 1 then return end
	...
	local function PhysCallback(ent, data)
		...
		if hitEnt == ent.Owner then return end
		if not IsValid(ent.Owner) or hitEnt:GetMoveType() == MOVETYPE_NOCLIP then return end
		if hitEnt:GetRD() and IsValid(hitEnt:GetRD()) then return end

		local impactVel = data.OurOldVelocity or Vector(0,0,0)
		local impactSpeed = impactVel:Length()
		local didJump = ent.GrabOverTime and ent.GrabOverTime > CurTime()
		if didJump and impactSpeed > 250 then

			ent.MuR_KnockdownCollisionTime = CurTime()
			hitEnt.MuR_KnockdownVictimTime = CurTime()
			local rag = hitEnt:StartRagdolling(0, 0, nil, impactVel)
			if IsValid(rag) then
				rag.MuR_KnockdownVictimTime = CurTime()
				hitEnt.IsRagStanding = false
				rag.CanStanding = false

				if math.random() < 0.3 then
					local wpn = hitEnt:GetActiveWeapon()
					if IsValid(wpn) and not wpn.NeverDrop then
						hitEnt:DropWeapon(wpn)
					end
				end
			end
		end
...
function pl:StartRagdolling(moans, dam, gibs, impactVel)
	if self:InVehicle() or self:IsExecuting() or self:IsInHostage(false) or self:GetNW2String("Class") == "Zombie" or self:GetNW2String("Class") == "Maniac" or self:GetNW2String("Class") == "Entity" or self:GetNW2String("Class") == "GeorgeDroidFloyd" or self:GetNW2String("Class") == "Tagila" or self:GetNW2Bool("GeroinUsed") or self:GetNW2Bool("FentanylUsed") or timer.Exists("MindControl_" .. self:EntIndex()) then return end
	if MuR.Gamemode == 54 and self:GetNW2String("Class") == "Combine" and self:GetNW2Int("CombineType", 1) == 1 then return end
	moans = moans or 0
	dam = dam or 0
	local ent = self:CreateAdvancedRagdoll(false, impactVel)
	...
	if IsValid(ent) then
		...
		local ragTime = math.Clamp(3 + dam / 10, 5, 10)
		self:TimeGetUpChange(ragTime, true)
...
function pl:UnconnectRagdoll(died)
	...
	if IsValid(rag) then
		...
		if died then
			rag.MuR_BloodWounds = table.Copy(self.BleedingWounds or {})
			rag.MuR_BloodBleedLevel = self:GetNW2Float("BleedLevel", 0)
		end
...
function ent:PullUpLogic()
	...
	if not IsValid(self.LeftHandGrab) and not IsValid(self.RightHandGrab) then
		self:JumpOutGrab()
		return
	end
...
hook.Add("EntityTakeDamage", "MuR.RagdollKnockdownNoCrush", function(ent, dmg)
	local dt = dmg:GetDamageType()
	if bit.band(dt, DMG_CRUSH) == 0 then return end
	local checkEnt = ent
	if ent.Owner and IsValid(ent.Owner) and ent.Owner:IsPlayer() then
		checkEnt = ent.Owner
	end
	local window = 1.0
	if checkEnt.MuR_KnockdownVictimTime and CurTime() - checkEnt.MuR_KnockdownVictimTime < window then
		dmg:SetDamage(0)
		return
	end
	if ent.MuR_KnockdownCollisionTime and CurTime() - ent.MuR_KnockdownCollisionTime < window then
		dmg:SetDamage(0)
		return
	end
	if ent.MuR_KnockdownVictimTime and CurTime() - ent.MuR_KnockdownVictimTime < window then
		dmg:SetDamage(0)
		return
	end
end)
...
hook.Add("Think", "MuR.RagdollDamage", function()
	for _, ply in player.Iterator() do
		...
		if IsValid(rag) and ply:Alive() then
			...
			local needCollision = ply.IsRagStanding or (rag.GrabOverTime and rag.GrabOverTime > CurTime())
			if needCollision then
				if rag:GetCollisionGroup() ~= COLLISION_GROUP_INTERACTIVE then rag:SetCollisionGroup(COLLISION_GROUP_INTERACTIVE) end
			else
				if rag:GetCollisionGroup() ~= COLLISION_GROUP_WEAPON then rag:SetCollisionGroup(COLLISION_GROUP_WEAPON) end
			end
			...
			if Head then
				...
				local maxExtent = 16384
				if math.abs(HeadPos.x) <= maxExtent and math.abs(HeadPos.y) <= maxExtent and math.abs(HeadPos.z) <= maxExtent then
					ply:SetPos(HeadPos)
				end
			end
...
hook.Add("PlayerDeath", "MuR.RagdollDamage", function(ply)
	if MuR.Gamemode == 54 and ply:GetNW2String("Class") == "Combine" and ply:GetNW2Int("CombineType", 1) == 1 then return end
	local rag = ply:GetRD()
	if IsValid(rag) then
...
hook.Add("CreateEntityRagdoll", "MuR.StampBloodOnDeathRagdoll", function(ply, rag)
	if not IsValid(rag) or not MuR.BroadcastStampBloodOnEntity then return end

	timer.Simple(0.15, function()
		if not IsValid(rag) then return end

		local wounds = rag.MuR_BloodWounds or {}
		for slot, wound in pairs(wounds) do
			if wound and wound.tier and wound.tier > 0 and wound.bone then
				local pos = MuR:BoneData(rag, wound.bone)
				if isvector(pos) then
					local scale = (wound.tier == 3 and 0.9) or (wound.tier == 2 and 0.7) or 0.5
					for _ = 1, 3 do
						MuR:BroadcastStampBloodOnEntity(rag, pos + VectorRand(-3, 3), scale * math.Rand(0.85, 1.15))
					end
				end
			end
		end
		if (rag.MuR_BloodBleedLevel or 0) > 0.5 and next(wounds) == nil then
			local torsoBones = {"ValveBiped.Bip01_Spine2", "ValveBiped.Bip01_Spine4"}
			for _, bone in ipairs(torsoBones) do
				local pos = MuR:BoneData(rag, bone)
				if isvector(pos) then
					MuR:BroadcastStampBloodOnEntity(rag, pos, 0.5)
				end
			end
		end

		rag.MuR_BloodWounds = nil
		rag.MuR_BloodBleedLevel = nil
	end)
end)