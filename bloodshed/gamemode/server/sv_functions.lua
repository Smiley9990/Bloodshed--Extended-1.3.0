...
util.AddNetworkString("MuR.TripwireDisarm")
util.AddNetworkString("MuR.TripwireDisarmResult")
...
util.AddNetworkString("MuR.Mode52StyleSelection")
util.AddNetworkString("MuR.Mode52StyleSelected")
util.AddNetworkString("MuR.Mode53TeamSelection")
util.AddNetworkString("MuR.Mode53TeamSelected")
util.AddNetworkString("MuR.Mode53VoteStats")
util.AddNetworkString("MuR.Mode53CloseMenu")
util.AddNetworkString("MuR.Mode53CloseStartScreen")
util.AddNetworkString("MuR.Mode54ClassSelection")
util.AddNetworkString("MuR.Mode54ClassSelected")
util.AddNetworkString("MuR.Mode54CloseClassMenu")
util.AddNetworkString("MuR.Mode54ClassCounts")
util.AddNetworkString("MuR.Mode54ClassRejected")
util.AddNetworkString("MuR.Mode56.MaSSkaMessage")
util.AddNetworkString("MuR.Mode56.ReinforcementMusic")
util.AddNetworkString("MuR.Mode56.StartMusic")
util.AddNetworkString("MuR.NextModeRoleAssign")
util.AddNetworkString("MuR.NextMode56Settings")
...
function meta:GiveWeapon(class, cantdrop)
	...
	local limitedWeapons = {
		["tfa_codww2k_molotov_vfire"] = true,
		["tfa_codww2_betty"] = true,
		["tfa_codww2_thermite_breach"] = true,
		["tfa_codww2_tear_gas"] = true,
	}

	if limitedWeapons[class] and self:HasWeapon(class) then
		return
	end
...
function MuR:Mode54PlayerSpawn(ply, class)
	local rebelModels = {"models/player/group03/male_01.mdl", "models/player/group03/male_02.mdl", "models/player/group03/male_03.mdl", "models/player/group03/male_04.mdl", "models/player/group03/male_05.mdl", "models/player/group03/male_06.mdl", "models/player/group03/male_07.mdl", "models/player/group03/male_08.mdl", "models/player/group03/male_09.mdl"}

	if class == "Combine" then
		local combineType = ply:GetNW2Int("CombineType", 0)
		if combineType < 1 or combineType > 5 then

			combineType = 4
			ply:SetNW2Int("CombineType", 4)
			MuR.Mode54CombineCounts = MuR.Mode54CombineCounts or {0, 0, 0, 0, 0}
			MuR.Mode54CombineCounts[4] = (MuR.Mode54CombineCounts[4] or 0) + 1
			for _, p in ipairs(player.GetAll()) do
				if IsValid(p) and p:GetNW2String("Class") == "Combine" then
					net.Start("MuR.Mode54ClassCounts")
					for i = 1, 5 do net.WriteUInt(MuR.Mode54CombineCounts[i] or 0, 8) end
					net.Send(p)
				end
			end
			timer.Simple(0.5, function()
				if IsValid(ply) and MuR.Gamemode == 54 then
					net.Start("MuR.Mode54ClassSelection")
					net.WriteBool(true)
					local counts = MuR.Mode54CombineCounts or {0, 0, 0, 0, 0}
					for i = 1, 5 do net.WriteUInt(counts[i] or 0, 8) end
					local limits = {1, 1, 1, 0, 0}
					for i = 1, 5 do net.WriteUInt(limits[i] or 0, 8) end
					net.Send(ply)
				end
			end)
		end
		...
		if combineType == 1 then
			...
			ply:SetModel("models/player_hl2_combine_wallhammer.mdl")
			...
			timer.Simple(0.2, function()
				if IsValid(ply) then
					local wep = ply:GetWeapon("tfa_bs_spas")
					if IsValid(wep) and wep.Primary and wep.Primary.Ammo then
						ply:GiveAmmo((wep.Primary.ClipSize or 8) * 8, wep.Primary.Ammo, true)
					else
			...
			timer.Simple(1, function()
				if IsValid(ply) then ply:EquipArmor("wallhammer_armor") end
			end)
		elseif combineType == 2 then

			ply:SetModel("models/player_hl2_combine_suppressor.mdl")
			...
			ply:SetWalkSpeed(100)
			ply:SetRunSpeed(280)
			ply:GiveWeapon("tfa_bs_m249")
			timer.Simple(0.2, function()
				if IsValid(ply) then
					local wep = ply:GetWeapon("tfa_bs_m249")
					if IsValid(wep) and wep.Primary and wep.Primary.Ammo then
						ply:GiveAmmo((wep.Primary.ClipSize or 100) * 4, wep.Primary.Ammo, true)
					else
						ply:GiveAmmo(400, "ar2", true)
					end
				end
			end)
			timer.Simple(1, function()
				if IsValid(ply) then ply:EquipArmor("suppressor_armor") end
			end)
		elseif combineType == 3 then

			ply:SetModel("models/player_combine_sniper.mdl")
			ply:SetMaxHealth(100)
			ply:SetHealth(100)
			ply:SetArmor(0)
			ply:SetWalkSpeed(100)
			ply:SetRunSpeed(260)
			ply:GiveWeapon("tfa_heavysniper")
			timer.Simple(0.2, function()
				if IsValid(ply) then
					local wep = ply:GetWeapon("tfa_heavysniper")
					if IsValid(wep) and wep.Primary and wep.Primary.Ammo then
						ply:GiveAmmo((wep.Primary.ClipSize or 5) * 12, wep.Primary.Ammo, true)
					else
						ply:GiveAmmo(60, "SniperRound", true)
					end
				end
			end)
			...
			timer.Simple(0.2, function()
				if IsValid(ply) then
					local wep = ply:GetWeapon("tfa_bs_usp")
					if IsValid(wep) and wep.Primary and wep.Primary.Ammo then
						ply:GiveAmmo((wep.Primary.ClipSize or 12) * 4, wep.Primary.Ammo, true)
					else
						ply:GiveAmmo(48, "Pistol", true)
					end
				end
			end)
			...
			timer.Simple(1, function()
				if IsValid(ply) then ply:EquipArmor("sniper_armor") end
			end)
		elseif combineType == 4 then

			ply:SetModel("models/player_hl2_combine_ordinal.mdl")
			ply:SetBodygroup(0, 0)
			ply:SetBodygroup(1, 0)
			ply:SetBodygroup(2, 0)
			ply:SetBodygroup(3, 0)
			...
			ply:SetWalkSpeed(100)
			ply:SetRunSpeed(280)
			local smgWeapons = {"tfa_bs_mp7", "tfa_bs_mp5a5", "tfa_bs_ump"}
			local primaryWeapon = table.Random(smgWeapons)
			ply:GiveWeapon(primaryWeapon)
			timer.Simple(0.2, function()
				if IsValid(ply) then
					local wep = ply:GetWeapon(primaryWeapon)
					if IsValid(wep) and wep.Primary and wep.Primary.Ammo then
						ply:GiveAmmo((wep.Primary.ClipSize or 30) * 6, wep.Primary.Ammo, true)
					end
				end
			end)
			ply:GiveWeapon("mur_m67")
			timer.Simple(1, function()
				if IsValid(ply) then ply:EquipArmor("ordinal_armor") end
			end)
		else

			ply:SetModel("models/player_hl2_combine_grunt.mdl")
			ply:SetMaxHealth(100)
			ply:SetHealth(100)
			ply:SetArmor(100)
			ply:SetWalkSpeed(100)
			...
			ply:GiveWeapon("mur_m67")
			timer.Simple(1, function()
				if IsValid(ply) then ply:EquipArmor("grunt_armor") end
			end)
		end

		if combineType ~= 3 then
		...
		ply:SetTeam(1)
		ply.Male = true
		ply:SetNW2Bool("CombineOverlay", true)
		timer.Simple(0.3, function()
			if IsValid(ply) and MuR.Gamemode == 54 then
				local wep = ply:GetWeapon("mur_radio")
				if IsValid(wep) then wep:SetNWInt("RadioChannel", 44) end
			end
		end)
	elseif class == "Rebel" then
		local rebelType = ply:GetNW2Int("RebelType", 0)
		if rebelType < 1 or rebelType > 5 then
			rebelType = 5
			ply:SetNW2Int("RebelType", 5)

			MuR.Mode54RebelCounts = MuR.Mode54RebelCounts or {0, 0, 0, 0, 0}
			MuR.Mode54RebelCounts[5] = (MuR.Mode54RebelCounts[5] or 0) + 1
			for _, p in ipairs(player.GetAll()) do
				if IsValid(p) and p:GetNW2String("Class") == "Rebel" then
					net.Start("MuR.Mode54ClassCounts")
					for i = 1, 5 do net.WriteUInt(MuR.Mode54RebelCounts[i] or 0, 8) end
					net.Send(p)
				end
			end
			timer.Simple(0.5, function()
				if IsValid(ply) and MuR.Gamemode == 54 then
					net.Start("MuR.Mode54ClassSelection")
					net.WriteBool(false)
					local counts = MuR.Mode54RebelCounts or {0, 0, 0, 0, 0}
					for i = 1, 5 do net.WriteUInt(counts[i] or 0, 8) end
					local limits = {1, 1, 1, 2, 0}
					for i = 1, 5 do net.WriteUInt(limits[i] or 0, 8) end
					net.Send(ply)
				end
			end)
		...
		if rebelType == 1 then
			...
			ply:GiveAmmo(180, "AR2", true)
			ply:GiveWeapon("tfa_bs_cobra")
			ply:GiveAmmo(24, "357", true)
			ply:GiveWeapon("tfa_bs_crowbar")
			ply:GiveWeapon("weapon_physcannon")
			timer.Simple(1, function()
				if not IsValid(ply) then return end
				ply:EquipArmor("hev_tors")
			end)
		elseif rebelType == 2 then

			ply:GiveWeapon("tfa_projecthl2_crossbow")
			ply:GiveAmmo(10, "XBowBolt", true)
			ply:GiveWeapon("tfa_bs_usp")
			ply:GiveAmmo(48, "Pistol", true)
			timer.Simple(1, function()
				if not IsValid(ply) then return end
				local rebel3HeadArmor = {"facecover_hockey_captain", "facecover_hockey_brawler", "facecover_hockey_quiet", "moto_helmet", "helmet_ulach", "helmet_tsh4m", "helmet_k1c", "helmet_sphera_c", "helmet_shpm", "cap_bear_black", "cap_bear_green", "cap_boss", "cap_mhs", "cap_police", "cap_scav", "helmet_riot"}
				local rebel3BodyArmor = {"vest_security", "vest_hexatac", "vest_korakulon_black", "vest_korakulon_emr", "vest_fishing"}
				ply:EquipArmor(table.Random(rebel3HeadArmor))
				ply:EquipArmor(table.Random(rebel3BodyArmor))
			end)
		elseif rebelType == 3 then

			ply:GiveWeapon("tfa_bs_uzi")
			timer.Simple(0.2, function()
				if IsValid(ply) then
					local wep = ply:GetWeapon("tfa_bs_uzi")
					if IsValid(wep) and wep.Primary and wep.Primary.Ammo then
						ply:GiveAmmo((wep.Primary.ClipSize or 30) * 6, wep.Primary.Ammo, true)
					end
				end
			...
			ply:GiveWeapon("tfa_bs_pm")
			ply:GiveAmmo(48, "Pistol", true)
			timer.Simple(1, function()
				if not IsValid(ply) then return end
				local rebel1HeadArmor = {"facecover_hockey_captain", "facecover_hockey_brawler", "facecover_hockey_quiet", "moto_helmet", "helmet_ulach", "helmet_tsh4m", "helmet_k1c", "helmet_sphera_c", "helmet_shpm", "cap_bear_black", "cap_bear_green", "cap_boss", "cap_mhs", "cap_police", "cap_scav", "helmet_riot"}
				local rebel1BodyArmor = {"vest_security", "vest_hexatac", "vest_korakulon_black", "vest_korakulon_emr", "vest_fishing", "classI_armor", "classII_armor"}
				ply:EquipArmor(table.Random(rebel1HeadArmor))
				ply:EquipArmor(table.Random(rebel1BodyArmor))
			end)
		elseif rebelType == 4 then

			local heavyWeapons = {"tfa_bs_m1014", "tfa_bs_spas", "tfa_bs_rpk", "tfa_bs_pkm"}
			local heavyWeapon = table.Random(heavyWeapons)
			ply:GiveWeapon(heavyWeapon)
			timer.Simple(0.2, function()
				if IsValid(ply) then
					local wep = ply:GetWeapon(heavyWeapon)
					if IsValid(wep) and wep.Primary and wep.Primary.Ammo then
						ply:GiveAmmo((wep.Primary.ClipSize or 30) * 8, wep.Primary.Ammo, true)
					end
				end
			end)
			timer.Simple(1, function()
				if not IsValid(ply) then return end
				local rebel2HeadArmor = {"helmet_tsh4m", "cap_bear_black", "cap_bear_green", "cap_boss", "cap_mhs", "cap_police", "cap_scav", "helmet_riot"}
				ply:EquipArmor(table.Random(rebel2HeadArmor))
				ply:EquipArmor("classIII_armor")
			end)
		else

			local primaryWeapons = {"tfa_bs_sks", "tfa_bs_ak12", "tfa_bs_ak74", "tfa_bs_akm", "tfa_bs_aks74u", "tfa_bs_aug", "tfa_bs_l1a1", "tfa_bs_g3", "tfa_bs_ump", "tfa_bs_uzi", "tfa_bs_vector", "tfa_bs_mac11", "tfa_bs_mp5a5", "tfa_bs_mp7"}
			local primaryWeapon = table.Random(primaryWeapons)
			ply:GiveWeapon(primaryWeapon)
			timer.Simple(0.2, function()
				if IsValid(ply) then
					local wep = ply:GetWeapon(primaryWeapon)
		...
		timer.Simple(0.3, function()
			if IsValid(ply) and MuR.Gamemode == 54 then
				local wep = ply:GetWeapon("mur_radio")
				if IsValid(wep) then wep:SetNWInt("RadioChannel", 45) end
			end
		end)
	end
end
...
function GM:PlayerSpawn(ply)
	if not ply.ForceSpawn then
		...
		local isMode53RoleAssignment = MuR.Gamemode == 53 and MuR.GameStarted and (ply:GetNW2String("Class") == "PMC" or ply:GetNW2String("Class") == "Wilds")
		if not MuR.GameStarted or !MuR.EnableDebug and MuR.GameStarted and MuR.TimeCount + 12 < CurTime() and not isMode53RoleAssignment then
	...
	ply:StripWeapons()
	ply:StripAmmo()
	ply:SetNW2Bool("CombineOverlay", false)
	local combineTimerName = "MuR_CombineWalking_" .. ply:EntIndex()
	if timer.Exists(combineTimerName) then
		timer.Remove(combineTimerName)
	end
	...
	if ply.ResetCriticalOrgans then ply:ResetCriticalOrgans() end
	...
	if MuR.Gamemode == 54 and MuR.Mode54 and (class == "Combine" or class == "Rebel") then
		...
		if isvector(actualBase) and actualBase ~= Vector(0, 0, 0) then
			...
			offset.z = 0
			pos = actualBase + offset + Vector(0, 0, 10)
		end
	end
	if not pos then
		pos = MuR:GetRandomPos()
	end
	...
	if MuR.Gamemode == 54 and (class == "Combine" or class == "Rebel") then
		MuR:Mode54PlayerSpawn(ply, class)
	else
	...
	timer.Simple(0.01, function()
		...
		if IsValid(att2) and att2:IsPlayer() then
			...
			elseif att2:GetNW2String("Class") != "Soldier" and (ply:Team() == 2 or ply:Team() == 3) and (att2:Team() == 2 or att2:Team() == 3) and MuR.Gamemode ~= 11 and MuR.Gamemode ~= 12 and ply ~= att2 then
				...
				if not (modeDef and modeDef.no_guilt) then
					local guiltEnabled = not GetConVar("mur_disableguilt"):GetBool()
					if ply.UnInnocentTime < CurTime() then
						att2:ChangeGuilt(4)
						if guiltEnabled then MuR:GiveAnnounce("innocent_kill", att2) end
					else
						if guiltEnabled then MuR:GiveAnnounce("innocent_att_kill", att2) end
					end
				end
			end
...
net.Receive("MuR.BodySearch", function(len, ply)
	...
	if string.StartWith(cl, "mur_armor_") then
		...
		if item then
			if item.permanent then return end
...
net.Receive("MuR.TripwireDisarmResult", function(len, ply)
	local ent = net.ReadEntity()
	...
	if ent:GetClass() == "murwep_grenade" then
		if success then
			if ent.CompleteDisarm and ent:CompleteDisarm(ply) then
				MuR:GiveMessage("tripwire_disarmed_success", ply)
			end
		else
			if ent.FailDisarm and ent:FailDisarm(ply) then
				MuR:GiveMessage("tripwire_disarmed_fail", ply)
			end
		end
	end
end)
...
hook.Add("PlayerButtonDown", "MuR_SButtons", function(ply, but)
	...
	if ply:Alive() and but == KEY_E and (not ply:GetNoDraw() or IsValid(ply:GetRD())) then
		...
		if IsValid(tr) and tr:GetClass() == "murwep_grenade" and IsValid(tr.StakeEnt) and tr:GetPos():DistToSqr(ply:GetPos()) < 10000 then
			if not tr.Activated then
				tr:DisarmTripwire(ply)
				return
			end
		end
...
hook.Add("PlayerCanHearPlayersVoice", "MuR.Voice", function(listener, talker)
	...
	if talker:Alive() and talker:GetNW2String("Class") == "Combine" and listener:Alive() and listener:GetNW2String("Class") == "Combine" then return true end
...
hook.Add("Think", "MuR_LogicPlayer", function()
	for _, ent in player.Iterator() do
		...
		if ent.HungerDelay < CurTime() and not MuR:DisablesGamemode() and MuR.Gamemode != 53 then
...
hook.Add("EntityTakeDamage", "MuR_DamageNPCThink", function(ent, dmg)
	...
	if ent:IsPlayer() then
		...
		if att:IsPlayer() and ent:IsPlayer() and MuR.Gamemode == 53 and att:Team() == ent:Team() then
			dmg:SetDamage(0)
			return true
		end
	end
	...
	if ent:IsPlayer() and ent:Alive() then
		...
		if att:IsPlayer() then
			...
			if att.DamageTargetGuilt <= 0 and MuR.Gamemode ~= 5 and MuR.Gamemode ~= 6 and MuR.Gamemode ~= 50 and MuR.Gamemode ~= 54 and (att:Team() == 2 or att:Team() == 3) then
...
hook.Add("AllowPlayerPickup", "MuR_WeaponsFuck", function(ply, ent)
	if ent:IsWeapon() then
		...
		if limitedWeapons[weaponClass] then
