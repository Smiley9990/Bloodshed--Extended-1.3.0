...
timer.Create("TryFindAINodes", 3, 0, function()
	...
	if VJ_Nodegraph and istable(VJ_Nodegraph.Data) and #VJ_Nodegraph.Data.Nodes > 0 then 
...
function MuR:GetRandomPos(underroof, frompos, mindist, maxdist, withoutply)
	...
	if #tab > 0 then
		for i = 1, #tab do
			...
			if MuR.IsPositionBlacklisted and MuR.IsPositionBlacklisted(pos) then continue end
	...
	elseif #visible_nodes > 0 then
		...
		for _, classname in ipairs(spawnClasses) do
			...
			if list and #list > 0 then
				...
				for _, ent in ipairs(list) do
					if IsValid(ent) then
						local pos = ent:GetPos() + Vector(math.Rand(-100, 100), math.Rand(-100, 100), 0)
						if not (MuR.IsPositionBlacklisted and MuR.IsPositionBlacklisted(pos)) then
		...
		for _, ply in ipairs(plys) do
			if IsValid(ply) and ply:Alive() then
				local pos = ply:GetPos() + Vector(math.Rand(-800, 800), math.Rand(-800, 800), 0)
				local tr = util.TraceLine({
					start = pos,
					endpos = pos - Vector(0, 0, 9999),
				})
				if tr.Hit then
					local hitPos = tr.HitPos + Vector(0, 0, 20)
					if not (MuR.IsPositionBlacklisted and MuR.IsPositionBlacklisted(hitPos)) then
						return hitPos
					end
				end
			end
...
function MuR:FindNearbySpawnPosition(basePos, maxDistance)
	...
	for i = 1, #MuR.AI_Nodes do
		...
		if distSqr <= maxDistance * maxDistance then
			if MuR.IsPositionBlacklisted and MuR.IsPositionBlacklisted(pos) then
	...
	if MuR.IsPositionBlacklisted and MuR.IsPositionBlacklisted(basePos) then
		local alt = MuR:GetRandomPos()
		if isvector(alt) then return alt end
	end
	return basePos
end
