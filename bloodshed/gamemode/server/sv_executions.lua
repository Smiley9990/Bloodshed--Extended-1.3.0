...
function meta:FullTakedown(forceent)
	...
	local execPos = self:GetPos()
	self:SetNW2Vector("ExecutionStartPos", execPos)
    ...
    tar:SetPos(execPos)
	tar:SetNW2Vector("ExecutionStartPos", execPos)
    tar:SetEyeAngles(self:EyeAngles())
	tar:SetVelocity(Vector(0,0,0))
    ...
    timer.Simple(dur-0.02, function()
        if IsValid(self) and self:Alive() and self:IsExecuting() then
            self:Freeze(false)
			self:SetNW2Vector("ExecutionStartPos", Vector(0,0,0))
...
hook.Add("PlayerPostThink", "MuR_Takedown", function(ply)
    if ply:IsExecuting() then
		...
		if (!IsValid(tar) or !tar:Alive() or !tar:IsExecuting()) and !IsValid(att) and !ply.CantBeStopped then
			...
			ply:SetNW2Vector("ExecutionStartPos", Vector(0,0,0))
			ply:SetSVAnimation("")
			ply:Freeze(false)
		end
    else
...
hook.Add("EntityTakeDamage", "MuR_StopExecution", function(ent, dmg)
	if ent:IsPlayer() and ent:IsExecutor() then
		ent:SetSVAnimation("")
		ent:Freeze(false)
		ent:SetNW2Vector("ExecutionStartPos", Vector(0,0,0))
		...
		if IsValid(ent.ExecutingTar) then
			...
			ent.ExecutingTar:SetNW2Vector("ExecutionStartPos", Vector(0,0,0))
