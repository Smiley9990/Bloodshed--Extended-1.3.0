...
SWEP.PrintName = "Illegal Chemistry Tablet"
...
if SERVER then
	util.AddNetworkString("mur_craft_anim")
	net.Receive("mur_craft_anim", function(len, ply)
		local isOpen = net.ReadBool()
		if isOpen then
			local wep = ply:GetActiveWeapon()
			if IsValid(wep) and wep.OpenAnim then
				wep:OpenAnim()
			end
		else
			local wep = ply:GetActiveWeapon()
			if IsValid(wep) and wep.CloseAnim then
				wep:CloseAnim()
			end
		end
	end)
...
function SWEP:Holster()
	if CLIENT then
		if IsValid(LocalPlayer().CraftingMenu) then
			LocalPlayer().CraftingMenu:Close()
		end
	end
	return true
end
