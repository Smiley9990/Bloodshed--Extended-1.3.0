        ...
        if self.Activated then
            ...
            if CurTime() > self.NextGasEmit then
                ...
                if IsValid(smoke) then
                    ...
                    smoke:SetKeyValue("startcolor", "150 150 150")
                    smoke:SetKeyValue("endcolor", "150 150 150")
                    smoke:SetKeyValue("opacity", "0.4")
                ...
                for _, ply in player.Iterator() do
                    if ply:GetPos():Distance(pos) < 350 then
                        if ply:Alive() then
                            MuR.SubstanceSystem:ApplyPoisonFromCloud(ply, "sarin", owner, 1)
                        end
                    end
                end
            end
